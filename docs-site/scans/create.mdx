...
The model-catalog response is cached and refreshed by the engine. For OpenRouter and xAI
it contains searchable choices returned through the configured key. The list is a discovery
aid, not an allowlist: automation may still submit an exact custom model ID.
...
| `model` | Yes | Provider model ID or supported alias. Use a current catalog entry, or an exact custom OpenRouter or xAI model ID. |
| `model_provider` | Yes | `codex`, `claude`, `openrouter`, or `xai`. |
| `harness` | Yes | Supported values are `codex`, `claude-code`, and `grok-build`; it must be compatible with the provider. |
...
Provider/harness combinations are `codex` + `codex`, `claude` + `claude-code`,
`openrouter` + `claude-code` or advanced `codex`, and `xai` + `grok-build`. The API
returns `422`
...
OpenRouter or xAI, use an ID returned by `/api/model-catalog` or an exact custom model ID. This
