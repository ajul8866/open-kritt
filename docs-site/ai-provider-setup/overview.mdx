...
| **xAI** | `XAI_API_KEY` | Searchable authenticated catalog, with an exact-ID fallback |
...
<CardGroup cols={2}>
  ...
  <Card title="xAI" icon="bolt" href="/ai-provider-setup/xai">
    Use an xAI API key with the Grok Build harness.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Choose one access method">
    ...
    **OpenRouter API key**, or **xAI API key**. Follow the provider guide above to obtain
    the correct credential. The CLI configures keys and Codex login; Claude subscription
    login and xAI/OpenRouter key management are also available from the running
...
OpenRouter and xAI keys synchronized with `.env`, and mirrors those keys under
...
[OpenRouter](/ai-provider-setup/openrouter), or [xAI](/ai-provider-setup/xai).
